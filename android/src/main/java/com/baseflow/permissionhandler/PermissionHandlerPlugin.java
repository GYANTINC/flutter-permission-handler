...
public class PermissionHandlerPlugin implements MethodCallHandler {
  ...
  @Retention(RetentionPolicy.SOURCE)
  @IntDef({ PERMISSION_STATUS_DENIED, PERMISSION_STATUS_DISABLED, PERMISSION_STATUS_GRANTED,
      PERMISSION_STATUS_RESTRICTED, PERMISSION_STATUS_UNKNOWN, })
  ...
  @Retention(RetentionPolicy.SOURCE)
  @IntDef({ SERVICE_STATUS_DISABLED, SERVICE_STATUS_ENABLED, SERVICE_STATUS_NOT_APPLICABLE, SERVICE_STATUS_UNKNOWN, })
  ...
  public void onMethodCall(MethodCall call, Result result) {
    ...
    case "checkPermissionStatus": {
      @PermissionGroup
      final int permission = (int) call.arguments;
      @PermissionStatus
      final int permissionStatus = checkPermissionStatus(permission);
    ...
    case "checkServiceStatus": {
      ...
      final int permission = (int) call.arguments;
      @ServiceStatus
      final int serviceStatus = checkServiceStatus(permission);
  ...
  private int checkPermissionStatus(@PermissionGroup int permission) {
    ...
    // if no permissions were found then there is an issue and permission is not set
    // in Android manifest
    if (names.size() == 0) {
    ...
    for (String name : names) {
      // Only handle them if the client app actually targets a API level greater than
      // M.
      if (targetsMOrHigher) {
  ...
  private void handlePermissionsRequest(String[] permissions, int[] grantResults) {
    ...
    for (int i = 0; i < permissions.length; i++) {
      @PermissionGroup
      final int permission = parseManifestName(permissions[i]);
      ...
      if (permission == PERMISSION_GROUP_MICROPHONE) {
        ...
        @PermissionStatus
        int permissionStatus = toPermissionStatus(grantResults[i]);
  ...
  private boolean isLocationServiceEnabled(Context context) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      ...
      final String locationProviders = Settings.Secure.getString(context.getContentResolver(),
